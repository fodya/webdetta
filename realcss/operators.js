    ...
    out.push({
      ...
      step: {
        ...
        build(rule) {
          ...
          rule.style = {
            transition: keys.map((k) => p + " " + toCssPropertyName(k)).join(
              ",",
            ),
          };
        },
...
/**
 * @param {(cell: unknown) => unknown} mount
 * @returns {Record<string, Function>}
 */
export const makeOperators = (mount) => {
  const wrap = (cell) => {
    cell[Element.lazy] = function () {
      return mount(cell);
    };
    return cell;
  };
  ...
  return {
    ...
    Select: (val, ...children) =>
      wrap({ kind: "mod", mod: { selector: val }, children }),
    Query: (val, ...children) =>
      wrap({ kind: "mod", mod: { query: val }, children }),
    Important: (...children) =>
      wrap({ kind: "mod", mod: { important: true }, children }),
    Inline: (...children) =>
      wrap({ kind: "mod", mod: { inline: true }, children }),

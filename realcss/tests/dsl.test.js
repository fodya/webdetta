...
const setupDom = async () => {
  ...
  if (!globalThis.CSS) {
    globalThis.CSS = { escape: (s) => String(s).replace(/[^\w-]/g, "_") };
  }
...
describe("realcss DSL v2", () => {
  ...
  describe("methodChain — persistence", () => {
    ...
    it("O(1) fork: getter allocates exactly one cons for pending", () => {
      ...
      const b = a.p;
      assert(
        b.pending.tail === a.pending,
        "new pending shares tail with source",
      );
    });
  ...
  describe("reactivity — end-to-end (no flag, auto-untrack)", () => {
    ...
    it("signal read via Plain(fn) triggers re-insertion on change", () => {
      ...
      color("green");
      assert(
        [...sheet.cssRules].some((rule) => rule.cssText.includes("green")),
      );
    });
  ...
  describe("mounting — inserts CSS rules into the stylesheet", () => {
    ...
    it("Select: inserts one rule with combined selector", () => {
      ...
      for (const r of sheet.cssRules) {
        if (r.cssText.includes(":hover") && r.cssText.includes("red")) {
          found = true;
        }
      }

...
/**
 * @param {unknown} head
 * @param {{ head: unknown, tail: unknown } | null} [tail]
 * @returns {{ head: unknown, tail: unknown | null }}
 */
...
export const validatePlainArg = (obj) => {
  ...
  throw new Error(
    `Plain expects plain object or function; got ${
      obj === null ? "null" : typeof obj
    }`,
  );
};

/** @type {{ selector: string, query: string, important: boolean, inline: boolean }} */
...
/**
 * @param {{ selector: string, query: string, important: boolean, inline: boolean }} parent
 * @param {{ selector?: string, query?: string, important?: boolean, inline?: boolean }} mod
 * @returns {{ selector: string, query: string, important: boolean, inline: boolean }}
 */
export const extendCtx = (parent, mod) => {
  ...
  ctx = Object.freeze({
    selector: mod.selector
      ? (parent.selector
        ? processNestedSelector(mod.selector, parent.selector)
        : mod.selector)
      : parent.selector,
...
class StyleRule {
  ...
  rebuild() {
    ...
    const body = processNestedSelector(ctx.selector, "." + this.cls) +
      styleStr(this.style, ctx.important);
...
/**
 * @param {unknown} step
 * @param {{ selector: string, query: string, important: boolean, inline: boolean }} ctx
 * @returns {StyleRule}
 */
 ...
 * @returns {{ proto: object, seed: (name: string) => Function }}
 */
export const MethodChain = (methods) => {
  ...
  for (const name of Object.keys(methods)) {
    Object.defineProperty(proto, name, {
      get() {
        return createChain(this.steps, cons(name, this.pending));
      },
    });
  }
  const createChain = (steps, pending) => {
    const fn = function (...args) {
      ...
      for (let i = names.length - 1; i >= 0; i--) {
        const name = names[i];
        next = cons(
          { kind: "method", name, method: methods[name], args },
          next,
        );
      }
 ...
 * @param {unknown} cell
 * @param {{ selector: string, query: string, important: boolean, inline: boolean }} ctx
 * @param {{ step: unknown, ctx: unknown }[]} out
 * @returns {void}
 */
...
/**
 * @param {{ insertNode: (rule: StyleRule) => void }} styleSheet
 * @returns {(cell: unknown) => unknown}
 */
export const makeMount = (styleSheet) => (cell) =>
  el.ref((dom) => {
    const insert = (rule) => {
      if (rule.inline) {
        for (const [k, v] of Object.entries(rule.style)) {
          dom.style.setProperty(toCssPropertyName(k), v);
        }
      } else {
        styleSheet.insertNode(rule);
        if (rule.cls) {
          dom.classList.add(rule.cls);
        }
      }
    };
    const remove = (rule) => {
      if (rule.inline) {
        for (const k of Object.keys(rule.style)) {
          dom.style.removeProperty(toCssPropertyName(k));
        }

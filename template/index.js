...
const Node = (operator) => ({ operator, args: [] });

const OP_REGEX = /[a-zA-Z_]/;

/**
 * @typedef {Object} TemplaterConfig
 * @property {string} operatorSymbol
 * @property {string} openBracket
 * @property {string} closeBracket
 * @property {string} argumentsSeparator
 * @property {(operator: string, parentNode: ParseNode) => void} [onOperatorNotFound]
 */

/**
 * @typedef {Object} ParseNode
 * @property {string | null} operator
 * @property {ParseArg[]} args
 */

/**
 * @typedef {string | ParseNode} ParseArg
 */

/**
 * @callback TemplaterRender
 * @param {ParseArg | ParseArg[]} node
 * @param {Record<string, unknown>} context
 * @returns {string}
 */

/**
 * @callback TemplaterOperatorHandler
 * @param {Record<string, unknown>} ctx
 * @param {ParseArg[]} args
 * @param {TemplaterRender} render
 * @returns {string}
 */

/**
 * @typedef {Object} TemplaterInstance
 * @property {(name: string, func: TemplaterOperatorHandler) => void} register
 * @property {(str: string) => ParseNode} parse
 * @property {(str: string, ctx: Record<string, unknown>) => string} render
 */
...
const parse = (str, config, operators, onOperatorNotFound) => {
  const { operatorSymbol, openBracket, closeBracket, argumentsSeparator } =
    config;
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (
      buf.endsWith(argumentsSeparator) && node !== root && nested === 0
    ) {
...
const defaultOnOperatorNotFound = (op) =>
  console.warn(
    `Templater: operator "${op}" is not registered, treating as literal`,
  );

/**
 * @param {TemplaterConfig} config
 * @returns {TemplaterInstance}
 */
